...
export function SiteHeader({ title, subtitle }: HeaderProps) {
  const unreadNotifications = announcements.filter((a) => a.priority === "high").length
  return (
    <header className="flex h-14 sticky top-0 shrink-0 items-center gap-2 border-b transition-[width,height] ease-linear group-has-data-[collapsible=icon]/sidebar-wrapper:h-(--header-height) bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
